...
import { WebRTCConnection } from "src/services/webrtc";
...
type IControls = { action: string; icon: any; onClick: () => void };
...
export const ExpandedChannel: React.FC<IExpandedChannel> = ({
  ...
  const getUserCamera = async() => {
    // console.log('bati')
     setUserVideo(prev => !prev);

    await webRTC.getUserCamera(videoRef);
  ...
  return (
    <>
      {isOpen ? (
        ...
        >
          <section className="bg-secondary-blue w-full max-w-4xl rounded-sm p-8 flex flex-col items-center gap-4 border-red-400 border relative">
            <ul className="w-full grid grid-cols-2 grid-rows-2 gap-4">
              {mockUsers.map((user, index) => (
                <li
                  key={user.id}
                  className=" h-56 bg-blue-800 flex flex-col items-center justify-center gap-4 rounded-md text-xl overflow-hidden"
                >
                  {userVideo ? (
                      <video
            ...
            <button
              ...
              className="absolute top-4 right-4 bg-blue-900 p-4 rounded-full border-slate-100 border-2"
            >
